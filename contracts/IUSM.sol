...
abstract contract IUSM {
    ...
    function ethBuffer(uint ethUsmPrice, uint ethInPool, uint usmSupply, WadMath.Round upOrDown) public virtual pure returns (int buffer);
    function debtRatio(uint ethUsmPrice, uint ethInPool, uint usmSupply) public virtual pure returns (uint ratio);
    function ethToUsm(uint ethUsmPrice, uint ethAmount, WadMath.Round upOrDown) public virtual pure returns (uint usmOut);
    function usmToEth(uint ethUsmPrice, uint usmAmount, WadMath.Round upOrDown) public virtual pure returns (uint ethOut);
    ...
    function usmPrice(Side side, uint ethUsmPrice, uint debtRatio_) public virtual view returns (uint price);
    function fumPrice(Side side, uint ethUsmPrice, uint ethInPool, uint usmSupply, uint fumSupply, uint adjustment) public virtual view returns (uint price);
